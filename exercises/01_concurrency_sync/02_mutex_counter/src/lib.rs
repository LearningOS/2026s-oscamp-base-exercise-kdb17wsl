...
pub fn concurrent_counter(n_threads: usize, count_per_thread: usize) -> usize {
    ...
    let shared_cnt: Arc<Mutex<usize>> = Arc::new(Mutex::new(0));
    let mut handles = Vec::new();

    for _i in 0..n_threads {
        let cnt = Arc::clone(&shared_cnt);
    ...
    let final_count = *shared_cnt.lock().unwrap();
    final_count
}
...
pub fn concurrent_collect(n_threads: usize) -> Vec<usize> {
    ...
    let mut x = (*shared_vec.lock().unwrap()).clone(); 
    x.sort();
    x
}
