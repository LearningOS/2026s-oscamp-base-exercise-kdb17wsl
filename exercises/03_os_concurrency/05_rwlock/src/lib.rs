...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        ...
        loop {
            ...
            if self.state.compare_exchange(state, new_state, Ordering::AcqRel, Ordering::Acquire,).is_ok() {
...
impl<T> Drop for RwLockWriteGuard<'_, T> {
    fn drop(&mut self) {
        if self.lock.state.compare_exchange(WRITER_HOLDING, 0, Ordering::Release, Ordering::Relaxed).is_err() {
